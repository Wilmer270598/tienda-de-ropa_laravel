...
<div class="container mx-auto p-4">
    <div style="background-color: #5583ab; padding: 30px; border-radius: 10px; color: #fff; max-width: 600px; margin: auto; box-shadow: 0 4px 10px rgba(0,0,0,0.2);">
        <h2 class="mb-4 text-center" style="font-weight: bold; font-size: 1.8rem;">Editar Producto</h2>
        ...
        <form action="{{ route('productos.update', $producto->id_producto) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="talla" class="form-control" value="{{ old('talla', $producto->talla) }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="id_temporada" class="form-select">
                    ...
                    @foreach($temporadas as $t)
                        <option value="{{ $t->id_temporada }}" {{ old('id_temporada', $producto->id_temporada) == $t->id_temporada ? 'selected' : '' }}>
                            {{ $t->nombre_temporada }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="id_proveedor" class="form-select">
                    ...
                    @foreach($proveedores as $p)
                        <option value="{{ $p->id_proveedor }}" {{ old('id_proveedor', $producto->id_proveedor) == $p->id_proveedor ? 'selected' : '' }}>
                            {{ $p->nombre_proveedor }}
                        </option>
                    @endforeach
                </select>
